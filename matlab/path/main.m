...
VEL=1;              %速度
w1=0.5;w2=0.3;w3=0.2;      %权重 w1-路径,w2-威胁,w3-惩罚
p_merge=0.4;        %交配概率
p_vari=0.4;         %变异概率
mu=0;sigma=1.5;     %变异的高斯分布的参数
...
for i_plane=1:plane_n
    ...
    for iteration=1:100
        ...
        newPathPopu1=select_roul(newPathPopu1,eva,bestPath1);
    ...
    for iteration=1:100
        ...
        newPathPopu2=select_roul(newPathPopu2,eva,bestPath2);
...
for iteration=1:60
    ...
    for i=1:plane_n
        ...
        %选择算子
        newtotalPopu(ista:iend,:,:)=select_roul(newtotalPopu(ista:iend,:,:),eva_coop(ista:iend),bestPath_c(i,:,:));
        %交配算子
